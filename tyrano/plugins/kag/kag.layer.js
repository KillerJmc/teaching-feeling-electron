...
tyrano.plugin.kag.layer = {
  ...
  init: function () {
    ...
    //分割用のレイヤ
    $("#tyrano_base")
      .append('<div id="root_layer_game" class="root_layer_game"></div>')
      .append('<div id="root_layer_system" class="root_layer_system"></div>');

    //隠しレイヤの登録
    //画面クリックのレイヤ
    const layer_obj_click = $("<div class='layer layer_event_click' style='z-index:9999;display:none'></div>");
    layer_obj_click.css({
    ...
    */
    ...
    //メニュー画面用のレイヤ
    const layer_menu = $("<div class='layer layer_menu' style='z-index:1000000000;display:none' align='center'></div>");
    layer_menu.css({
      "width": this.kag.config.scWidth,
      "height": this.kag.config.scHeight,
      "position": "absolute"
    ...
    //フリーレイヤ
    const layer_free = $("<div class='layer layer_free' style='z-index:9998;display:none' ></div>");
    layer_free.css({
      "width": this.kag.config.scWidth,
      "height": this.kag.config.scHeight,
      "position": "absolute"
  ...
  addLayer: function (layer_name) {
    ...
    const layer_obj_fore = $("<div class='layer " + layer_name + "_fore layer_fore'></div>");
    const layer_obj_back = $("<div class='layer " + layer_name + "_back layer_back' style='display:none'></div>");
  ...
  hideMessageLayers: function () {
    ...
    const num_message_layer = parseInt(this.kag.config.numMessageLayers);

    for (let i = 0; i < num_message_layer; i++) {
  ...
  showMessageLayers: function () {
    ...
    for (let i = 0; i < num_message_layer; i++) {
      const j_layer = this.getLayer("message" + i);
  ...
  refMessageLayer: function (target_layer) {
    ...
    if (!target_layer) {
      ...
      for (let num = 0; this.map_layer_fore["message" + num]; num++) {
        ...
        j_message_inner.css({
          "left": parseInt(j_message_outer.css("left")) + 10,
          "top": parseInt(j_message_outer.css("top")) + 10,
          "width": parseInt(j_message_outer.css("width")) - 10,
          "height": parseInt(j_message_outer.css("height")) - 10
        });

      }

    } else if (this.map_layer_fore[target_layer]) {
  ...
  getLayerHtml: function () {
    ...
    $(".blendlayer").each(function () {
      layer_info.layer_blend[n] = $.playerHtmlPath($(this).outerHTML());
      n++;
    });
  ...
  setLayerHtml: function (layer) {
    ...
    for (const key in layer.map_layer_fore) {
    ...
    //fixlayer は復元しない
    for (const key in layer.layer_fix) {
      $("#tyrano_base").append($(layer.layer_fix[key]));
    }

    //ブレンド演出の復元
    $(".blendlayer").remove();
    for (const key in layer.layer_blend) {

      const obj = $(layer.layer_blend[key]);
      if (obj.hasClass("blendvideo")) {
        //ビデオの再現
        //console.log(obj.attr("data-video-pm"));
        const video_pm = JSON.parse(obj.attr("data-video-pm"));

        video_pm.stop = "true";
        video_pm.time = 10;
        //ビデオレイヤ追加だお。
        (function () {
          const _video_pm = video_pm;
          setTimeout(function () {
  ...
  backlay: function (layer) {
    ...
    for (const key in this.map_layer_fore) {
      if (layer == "" || layer == key) {
        ...
        const fore_class_name = this.map_layer_fore[key].attr("class");
        let back_class_name = this.map_layer_back[key].attr("class");

        this.map_layer_back[key] = this.map_layer_fore[key].clone();

        this.map_layer_back[key]
          .removeClass(fore_class_name)
          .addClass(back_class_name);
  ...
  forelay: function (layer) {
    ...
    for (const key in this.map_layer_back) {
      if (layer == "" || layer == key) {
        ...
        let fore_class_name = this.map_layer_fore[key].attr("class");
        const back_class_name = this.map_layer_back[key].attr("class");

        this.map_layer_fore[key] = this.map_layer_back[key].clone(true);

        this.map_layer_fore[key]
          .removeClass(back_class_name)
          .addClass(fore_class_name);
