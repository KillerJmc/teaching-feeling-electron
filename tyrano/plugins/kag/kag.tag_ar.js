...
tyrano.plugin.kag.tag.bgcamera = {
  ...
  start: function (pm) {
    ...
    if (pm.height != "") {
      ...
      if (pm.fit == "true") {
        ...
        const scWidth = parseInt(this.kag.config.scWidth);
        const scHeight = parseInt(this.kag.config.scHeight);
    ...
    const media = navigator.mediaDevices.getUserMedia(opt);
  ...
  checkPicture: function (video, pm) {
    ...
    const scWidth = parseInt(this.kag.config.scWidth) / 4;
    const scHeight = parseInt(this.kag.config.scHeight) / 4;
      ...
      if (url.indexOf("http") != -1) {
        ...
        if (mode == "all" || mode == "define" || mode == "web") {
          ...
          if (this.kag.stat.qr.define[url]) {
            ...
            const jump_pm = this.kag.stat.qr.define[url];
