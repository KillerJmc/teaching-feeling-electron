...
tyrano.plugin.kag.ftag = {
  ...
  showNextImg: function () {
    ...
    if (this.kag.stat.flag_glyph == "false") {
      $(".img_next").remove();
      const jtext = this.kag.getMessageInnerLayer();
  ...
  checkVital: function (tag) {
    ...
    const array_vital = master_tag.vital;

    for (let i = 0; i < array_vital.length; i++) {
      const vital = array_vital[i];
  ...
  nextOrderWithLabel: function (label_name, scenario_file) {
    ...
    if (label_name == "*savesnap") {
      ...
      const tmpsnap = this.kag.menu.snap;

      const co = tmpsnap.current_order_index;
      const cs = tmpsnap.stat.current_scenario;
  ...
  start: function (pm) {
    ...
    if (this.kag.stat.vertical == "true") {
      ...
      if (this.kag.config.defaultAutoReturn != "false") {
        ...
        const limit_width = parseInt(j_outer_message.css("width")) * 0.8;
        const current_width = parseInt(j_inner_message.find("p").css("width"));
      ...
      if (this.kag.config.defaultAutoReturn != "false") {
        ...
        const limit_height = parseInt(j_outer_message.css("height")) * 0.8;
        const current_height = parseInt(j_inner_message.find("p").css("height"));
      ...
      if (that.kag.stat.fuki.active) {
        ...
        if (chara_name == "") {
          ...
          if ((typeof chara_obj != "undefined" && chara_obj.get(0)) && that.kag.stat.charas[original_name]["fuki"]["enable"] == "true") {
            ...
            const j_outer_message = that.kag.getMessageOuterLayer();
        ...
        if (is_chara_show == false) {
          ...
          const others_style = that.kag.stat.fuki.others_style;
          const def_style = that.kag.stat.fuki.def_style;

          const nwidth = others_style.max_width || def_style.width;
          const nleft = others_style.left || def_style.left;
          const ntop = others_style.top || def_style.top;
...
tyrano.plugin.kag.tag.fuki_start = {
  ...
  start: function (pm) {
    ...
    const j_msg_inner = this.kag.layer.getLayer(pm.layer, pm.page).find(".message_inner");
    j_msg_inner.css("width", "");
    j_msg_inner.css("height", "");
...
tyrano.plugin.kag.tag.fuki_stop = {
  ...
  start: function (pm) {
    ...
    const j_outer_layer = this.kag.getMessageOuterLayer();
    ...
    const j_inner_layer = this.kag.getMessageInnerLayer();

    j_inner_layer.css({
      "max-width": "",
      "left": parseInt(j_outer_layer.css("left")) + 10,
      "top": parseInt(j_outer_layer.css("top")) + 10
    });

    //スタイルをもとに戻す
    const def_style_inner = this.kag.stat.fuki.def_style_inner;
  ...
  start: function (pm) {
    ...
    if (pm.overwrite == "true" && pm.name != "") {
      ...
      if (pTextSelector.length > 0) {
        ...
        if (pm.x != 0) {
          pTextSelector.css("left", parseInt(pm.x));
        }

        if (pm.y != 0) {
          pTextSelector.css("top", parseInt(pm.y));
        }
        ...
        if (pm.size != "") {
          pTextSelector.css("font-size", parseInt(pm.size));
        }
...
tyrano.plugin.kag.tag.trans = {
  ...
  start: function (pm) {
    ...
    // let layer_num = $.countObj(this.kag.layer.map_layer_fore);
    //
