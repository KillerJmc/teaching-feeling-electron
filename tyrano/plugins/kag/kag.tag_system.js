...
:param
exp=評価するTJS式を指定します。
next=Internal variable used in the [autoconfig] tag. Calling [eval] with next=false will prevent it from running the next macro/tag from the macro/tag queue.
...
tyrano.plugin.kag.tag.trace = {
  ...
  start: function (pm) {
    const val = this.kag.embScript(pm.exp);
    //評価された値を代入
...
tyrano.plugin.kag.tag.endhtml = {
  ...
  start: function (pm) {
    ...
    const tpm = this.kag.stat.map_html.buff_pm;
    const html = this.kag.stat.map_html.buff_html;

    const html_obj = $("<div></div>");
    html_obj.css({
      "position": "absolute",
      "top": tpm.top + "px",
      "left": tpm.left + "px"
    ...
    /*layer_free.off("click");
    layer_free.on("click",function(){
      that.kag.ftag.nextOrder();
      layer_free.off("click");
    });*/

    layer_free.css("z-index", 9999999).append(html_obj).show();
...
tyrano.plugin.kag.tag.emb = {
  ...
  start: function (pm) {
    ...
    // edit - enable clicking during the time when text is added on the screen
...
tyrano.plugin.kag.tag.if = {
  ...
  start: function (pm) {
    ...
    if (this.kag.embScript(pm.exp)) {
      ...
      let i = 0;
      for (; i < 2000; i++) {
        const r = this.kag.ftag.nextOrderWithTag({
          "else": "",
          "elsif": "",
          "endif": ""
        });
      ...
      if (i > 1900) {
        ...
        $.error_message(`Error: ${current_storage}, line: ${line}\nIf statement contains an error [if]`);
      }
    }
  }
};
  ...
  start: function (pm) {
    ...
    if (this.kag.getStack("if").bool == false && this.kag.embScript(pm.exp)) {
      ...
      if (i > 1900) {
        ...
        const current_storage = this.kag.stat.current_scenario;
        const line = parseInt(this.kag.stat.current_line) + 1;
        $.error_message(`Error: ${current_storage}, line: ${line}\nIf statement contains an error [elsif]`);
      }
  ...
  start: function (pm) {
    ...
    if (this.kag.getStack("if").bool == false) {
      ...
      if (i > 1900) {
        ...
        const current_storage = this.kag.stat.current_scenario;
        const line = parseInt(this.kag.stat.current_line) + 1;
        $.error_message(`Error: ${current_storage}, line: ${line}\nIf statement contains an error [else]`);
      }
...
tyrano.plugin.kag.tag.return = {
  ...
  start: function () {
    ...
    const pm = this.kag.getStack("call");
    //最新のコールスタックを取得
    //呼び出し元に戻る

    //make.ksが終わるときの判定用
    if (pm.caller && pm.caller.storage == "make.ks") {
...
tyrano.plugin.kag.tag.macro = {
  ...
  start: function (pm) {
    ...
    const index = this.kag.ftag.current_order_index;
    const storage = this.kag.stat.current_scenario;
    ...
    if (i > 1900) {
      ...
      this.kag.error("macro " + pm.name + " is not closed with [endmacro]");
    }
...
tyrano.plugin.kag.tag.autoload = {
  ...
  start: function (pm) {
    const game_data = $.getStorage(this.kag.config.projectID + "_tyrano_auto_save", this.kag.config.configSave);
  ...
  start: function (pm) {
    ...
    if (this.kag.embScript(pm.exp)) {
      ...
      if (i > 1900) {
        ...
        this.kag.error("[ignore] is not closed with [endignore]");
      }
    } else {
      this.kag.ftag.nextOrder();
    }
...
tyrano.plugin.kag.tag.edit = {
  ...
  start: function (pm) {
    ...
    const j_text = $("<input class='text_box form' name='" + pm.name + "' type='text' value='' />");
    ...
    j_text.css(new_style).attr("maxlength", pm.maxchars);
    ...
    this.kag.layer.getFreeLayer().append(j_text).show();
  ...
  setEvent: function (j_text, pm) {
    ...
    (function () {
      ...
      j_text.on("click", function () {
        j_text.trigger('focus');
      });
...
tyrano.plugin.kag.tag.commit = {
  ...
  start: function () {
    ...
    this.kag.layer.getFreeLayer().find(".form").each(function () {
      ...
      const val = $(this).val();
      // fix for backslashes and apostrophes in inputs
      const str = name + " = '" + val.replace(/\\/g, "\\\\").replace(/'/g, "\\'") + "'";
...
tyrano.plugin.kag.tag.sleepgame = {
  ...
  start: function (pm) {
    ...
    const title = this.kag.stat.current_save_str;

    //save tf so we can restore it on awakegame
    this.kag.variable.before_sleepgame_tf = $.extend(true, {}, this.kag.variable.tf);
...
tyrano.plugin.kag.tag.awakegame = {
  ...
  pm: {
    variable_over: "true",
    bgm_over: "true",
    do_not_run_make_ks: false /* added this to fix visual bugs after awakegame */
  },
  ...
  start: function (pm) {
    ...
    if (this.kag.tmp.sleep_game == null) {
      ...
      const sleep_data = this.kag.tmp.sleep_game;
      ...
      //fix - don't run make.ks on awakegame if run_make_ks == true
      if (pm.do_not_run_make_ks == "true") {
        options.do_not_run_make_ks = true;
      }
...
tyrano.plugin.kag.tag.apply_local_patch = {
  ...
  start: function (pm) {
    ...
    const patch_path = $.localFilePath() + "/" + pm.file;
...
tyrano.plugin.kag.tag.check_web_patch = {
  ...
  checkPatch: function (obj, pm) {
    ...
    if (parseFloat(this.kag.variable.sf._patch_version) < parseFloat(obj.version)) {
      ...
      $.confirm("A new update has been found. Ver: " + parseFloat(obj.version) + "「" + obj.message + "」<br />Do you want to update?",
        function () {

          // alert("アップデートを行います。完了後、自動的にゲームは終了します。");
          alert("Updating. After its done, the game will close automatically.");
          ...
          const file = obj.file;
          // URLを指定
          const url = $.getDirPath(pm.url) + file;
          ...
          // 出力ファイル名を指定
          let patch_path = $.localFilePath();
          patch_path = patch_path + "/" + file;

          const outFile = fs.createWriteStream(patch_path);
