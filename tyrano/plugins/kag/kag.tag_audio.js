...
The audio files need to be stored in the 'bgm' folder.
...
:param
storage=The audio file to be played
loop=true (default) or false. true will loop the track
...
tyrano.plugin.kag.tag.playbgm = {
  ...
  pm: {
    ...
    volume: "",
    buf: "0",
    target: "bgm", // internal variable - "bgm" or "se"
  ...
  play: function (pm) {
    ...
    if (pm.target == "se") {
      ...
      if (pm.stop == "false") {
        ...
        } else if (this.kag.stat.current_se[pm.buf]) {
    ...
    // a shitty file check so howler won't fucking soft-lock the game
    ...
    $.ajax({
      ...
      cache: false,
        ...
        if (pm.sprite_time != "") {
          ...
          const sprite_from = parseInt($.trim(array_sprite[0]));
          const sprite_to = parseInt($.trim(array_sprite[1]));
        ...
        //音楽再生
        const audio_obj = new Howl(howl_opt);
        ...
        if (pm.loop != "true") {
          ...
          audio_obj.on("end", function (e) {
            ...
            if (pm.target == "se") {
              ...
              // edit - sound queue
              if (pm.queue && pm.queue.length) {
                const next_sound_pm = $.extend(true, {}, pm);
                next_sound_pm.storage = next_sound_pm.queue.shift();
                if (!next_sound_pm.queue.length && (next_sound_pm.last_in_queue_should_loop === "true" || next_sound_pm.last_in_queue_should_loop === true)) {
                  next_sound_pm.loop = true;
                }
                that.playQueue(next_sound_pm);
              }
      ...
      }.bind(this) //bind(this) to pass the context

    ).fail(function () {
        const error = "Audio file 「" + storage_url + "」 not found.";
        if (that.kag.config["debugMenu.visible"] === "true") {
          $.error_message(error);
        } else {
          console.error(error);
        }
  ...
  playQueue: function (pm) {
    ...
    if (pm.volume != "") {
      volume = parseFloat(pm.volume) / 100.0;
    }
    ...
    if (typeof this.kag.config.defaultSeVolume == "undefined") {
      ...
      ratio = parseFloat(this.kag.config.defaultSeVolume) / 100.0;
    }

    //bufが指定されていて、かつ、デフォルトボリュームが指定されている場合は
    if (typeof this.kag.stat.map_se_volume[pm.buf] != "undefined") {
      ratio = parseFloat(this.kag.stat.map_se_volume[pm.buf]) / 100.0;
    }
    ...
    if ($.isHTTP(pm.storage)) {
      ...
      if (storage != "") {
        storage_url = "./data/sound/" + storage;
      } else {
        storage_url = "";
      }
    }
    ...
    $.ajax({
      ...
      cache: false,
        ...
        if (pm.sprite_time != "") {
          const array_sprite = pm.sprite_time.split("-");
          const sprite_from = parseInt(array_sprite[0]);
          const sprite_to = parseInt(array_sprite[1]);
          const duration = sprite_to - sprite_from;
        ...
        const error = "Queue: audio file 「" + storage_url + "」 not found.";
        if (that.kag.config["debugMenu.visible"] === "true") {
          $.error_message(error);
        } else {
          console.error(error);
...
loop=true (default) or false を指定してください。trueを指定すると繰り返し再生されます,
sprite_time=再生の開始時間と終了時間を指定することができます。ミリ秒で指定します。例えば 6000-10000 と指定すると 6:00〜10:00　の間のみ再生できます。loopがtrueの場合はこの間のみループ再生します。,
html5=true or falseを指定。デフォルトはfalse。trueを指定するとHTML5 Audioで再生します。通常はWeb Audio API での再生です。特に指示がない場合はそのままでOKです。,
...
loop=true or false (default) を指定してください。trueを指定すると繰り返し再生されます,
sprite_time=再生の開始時間と終了時間を指定することができます。ミリ秒で指定します。例えば 6000-10000 と指定すると 6:00〜10:00　の間のみ再生できます。loopがtrueの場合はこの間のみループ再生します。,
clear=true or false (default) 他のSEが鳴っている場合、trueだと他のSEを停止した後、再生します。音声などはtrueが便利でしょう,
html5=true or falseを指定。デフォルトはfalse。trueを指定するとHTML5 Audioで再生します。通常はWeb Audio API での再生です。特に指示がない場合はそのままでOKです。,
...
tyrano.plugin.kag.tag.playse = {
  ...
  pm: {
    ...
    // edit - sound queue
    queue: [], // usage: ['path_to_snd2', 'path_to_snd3'] ('snd1' is already defined in the storage parameter)
    last_in_queue_should_loop: false
  },
...
tyrano.plugin.kag.tag.fadeoutse = {
  ...
  pm: {
    target: "se", // internal variable
    buf: "0",
    fadeout: "true" // internal variable
  },
...
tyrano.plugin.kag.tag.bgmopt = {
  ...
  start: function (pm) {
    ...
    if (pm.effect == "true" && this.kag.define.FLAG_APRI == false) {
      ...
      const new_volume = parseFloat(parseInt(pm.volume) / 100);
...
tyrano.plugin.kag.tag.voconfig = {
  ...
  start: function (pm) {
    ...
    // const map_vo = this.kag.stat.map_vo;
...
tyrano.plugin.kag.tag.speak_on = {
  ...
  start: function (pm) {
    ...
    if ('speechSynthesis' in window) {
      ...
      console.error("*error:This environment does not support the [speak_on] reading function");
    }
