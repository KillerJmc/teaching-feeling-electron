...
tyrano.plugin.kag = {
  ...
  stat: {
    ...
    current_bgm_html5: true, //現在再生中のhtml5パラメータ
    ...
    flag_ref_page: false, //if this flag is set, the next click will clear the screen.
  ...
  init: function () {
    ...
    $("script").each(function () {
      const att_src = $(this).attr("src");
      if (att_src) {
        if (att_src.indexOf("cordova") != -1 || att_src.indexOf("phonegap") != -1) {
    ...
    //ティラノプレイヤーを使ってるなら
    if (typeof TyranoPlayer == "function" || $.isNWJS() || $.isElectron()) {
  ...
  checkUpdate: function (call_back) {
    ...
    const patch_path = $.localFilePath() + "/" + this.kag.config.projectID + ".tpatch";
  ...
  applyPatch: function (patch_path, flag_reload, call_back) {
    ...
    // asar化している場合は上書きできない
    // Cannot be overwritten when asarized
    if (unzip_path == "asar") {
      ...
      if (process.platform == "darwin") {
        ...
        alert("Select the location of the game executable (.app) to which you want to apply the patch.");

        //実行パスを選択させる
        const { ipcRenderer } = require('electron');
        const filenames = ipcRenderer.sendSync('darwin-select-app-sync');

        if (typeof filenames == "undefined") {
          // alert("パッチの適応を中止します");
          alert("Patching stopped.");
          call_back();
          return;
        }
      ...
      // fix for shitty android webviews that don't know what async is
      asar.extractAll(_path.resolve(path), _path.resolve(out_path + "/update_tmp/")).then(() => {
        ...
        asar.createPackage(src, dest).then(() => {
          ...
          $.alert("Patch applied. Please restart the game.", function () {
            //パッチの削除。
      ...
      $.alert("Patch applied. Restarting...", function () {
        location.reload();
      });
  ...
  clearTmpVariable: function () {
    const tmp_sys = this.kag.variable.tf.system;
...
///スタック管理用
  ...
  endStorage: function () {
    ...
    const pm = this.kag.getStack("call"); //最新のコールスタックを取得
    //呼び出し元に戻る
...
/////////////ゲーム初期化////////////
  ...
  init_game: function () {
    ...
    if ($.isElectron() && that.kag.config.configSave == "file") {
      ...
      if (localStorage.getItem(that.save_key_id)) {
        ...
        //セーブデータ上書き
        const tmp_array2 = that.menu.getSaveData();
        //ハッシュを上書き
        tmp_array2["hash"] = that.save_key_val;
        $.setStorage(that.kag.config.projectID + "_tyrano_data", tmp_array2, that.kag.config.configSave);

      }

      //ハッシュに差分があったら、警告を表示して上書きするか確認。
      const tmp_array = that.menu.getSaveData();
    ...
    //システム変数の初期化
    const tmpsf = $.getStorage(this.kag.config.projectID + "_sf", that.config.configSave);
    ...
    //自動セーブのデータがあるかどうか
    const auto_save_data = $.getStorage(this.kag.config.projectID + "_tyrano_auto_save", this.kag.config.configSave);
    ...
    //コンフィグボタン追加
    const button_menu_obj = $("<div class='button_menu' style='z-index:100000000'><img src='./tyrano/images/system/" + $.novel("file_button_menu") + "'  /></div>");

    //コンフィグボタンの位置を指定する

    if (this.kag.config.configLeft != "-1" && this.kag.config.configTop != "-1") {
      button_menu_obj.css({
        left: parseInt(this.kag.config.configLeft),
        top: parseInt(this.kag.config.configTop)
      });
    } else {
      button_menu_obj.css({
        left: this.config.scWidth - 70,
        top: this.config.scHeight - 70
      });
    }

    button_menu_obj.on("click", function () {
      that.menu.showMenu();
    });
    ...
    //メッセージ外枠部分の作成
    const j_message = $("<div class='message_outer'></div>");
    j_message.css({
      "background-color": $.convertColor(this.config.frameColor),
      "opacity": $.convertOpacity(this.config.frameOpacity),
      "left": eval(this.config.ml),
      "top": eval(this.config.mt),
      "width": eval(this.config.mw),
      "height": eval(this.config.mh),
      "z-index": 100
    });
    ...
    //メッセージ表示領域
    const j_message_inner = $("<div class='message_inner' style='z-index:1001'></div>");
    ...
    const num_message_layer = parseInt(this.config.numMessageLayers);

    for (let i = 1; i < num_message_layer; i++) {
      const message_layer_name = "message" + i;

      this.layer.addLayer(message_layer_name);
      //２個め移行はデフォルト非表示
      this.layer.getLayer(message_layer_name).attr("l_visible", "false").hide();
      const j_message1 = j_message.clone(false);

      this.layer.appendObj(message_layer_name, "fore", j_message1);
      const j_message_inner1 = j_message_inner.clone(false);
    ...
    const fore_layer_num = parseInt(this.config.numCharacterLayers);
    for (let i = 0; i < fore_layer_num; i++) {
      this.layer.addLayer("" + i);
      this.layer.getLayer("" + i, "fore")
        .css({
          "display": "none",
          "z-index": 10 + i //デフォルト非表示　前景レイヤ
        });
    ...
    //文字のアンチエイリアス効果
    const smooth = this.kag.config.defaultAntialiased; //アンチエイリアス効果
    ...
    if (this.kag.config["vchatMenuVisible"] && this.kag.config["vchatMenuVisible"] == "true") {
      ...
      let player_back_cnt = 0;

      //コンフィグを表示する。
      setTimeout(function () {

        (function () {
          const j_menu_button = $("<div id='player_menu_button' class='player_menu_area' style='display:none;opacity:0.6;border-radius:5px;padding:10px;margin:10px;cursor:pointer;position:absolute;left:0px;top:0px;background-color:white;font-size:2em'><span style='color:#6495ED'>Menu</span></div>");
          const j_menu_area = $("<div style='display:none;position:absolute;left:10px;top:10px;font-size:2em'></div>");

          const j_end_button = $("<div class='player_menu_area' id='player_end_button' style='opacity:0.6;border-radius:5px;padding:10px;margin:10px 10px 10px 10px;cursor:pointer;left:0px;top:0px;background-color:white;'><span style='color:#6495ED'>タイトルへ</span></div>");
          const j_auto_button = $("<div class='player_menu_area' id='player_auto_button' style='opacity:0.6;border-radius:5px;padding:10px;margin:10px 10px 10px 10px;cursor:pointer;left:0px;top:0px;background-color:white;'><span style='color:#6495ED'>オート</span></div>");
          const j_skip_button = $("<div class='player_menu_area' id='player_skip_button' style='opacity:0.6;border-radius:5px;padding:10px;margin:10px 10px 10px 10px;cursor:pointer;left:0px;top:0px;background-color:white;'><span style='color:#6495ED'>スキップ</span></div>");

          j_menu_area.append(j_end_button)
            .append(j_auto_button)
            .append(j_skip_button);
          ...
          j_menu_button.on("click", function (e) {
            j_menu_button.hide();
            j_menu_area.show();
            /*
            j_end_button.show();
          ...
          j_end_button.on("click", function (e) {
          ...
          j_auto_button.on("click", function (e) {
          ...
          j_skip_button.on("click", function (e) {
          ...
          $("body").append(j_menu_button).append(j_menu_area);
    ...
    let first_scenario_file = "first.ks";

    const first_scenario_file_selector = $("#first_scenario_file");
    if (first_scenario_file_selector.length > 0) {
      first_scenario_file = first_scenario_file_selector.val();
    }
  ...
  pushBackLog: function (str, type) {
    ...
    const max_back_log = parseInt(this.kag.config["maxBackLogNum"]);
    ...
    if (type === "join") {
      ...
      if (index >= 0) { //配列が存在しない場合はpushだ
        const tmp = this.variable.tf.system.backlog[index];
  ...
  cutTimeWithSkip: function (time) {
    if (this.kag.stat.is_skip == true) {
      if (this.kag.config.skipEffectIgnore == "true") {
        ...
        // fix - increased this from 70 to 250
        // fixes numerous bugs with chara position, size and even images not showing up while skipping in TF
        // with 70 e.g. the 'act' window may be invisible after skipping
        return 250;
      }
    }
    return time;
  ...
  updateFuki: function (chara_name, opt = {}) {
    ...
    // const msg_inner_layer = this.kag.getMessageInnerLayer();
    const msg_outer_layer = this.kag.getMessageOuterLayer();
    ...
    const fuki_chara = this.kag.stat.charas[chara_name]["fuki"];
  ...
  loadScenario: function (file_name, call_back) {
    ...
    if (that.cache_scenario[file_url]) {
      if (call_back) {
        const result_obj = that.cache_scenario[file_url];
        const tag_obj = result_obj.array_s;
      ...
      $.loadText(file_url, function (text_str) {
        const result_obj = that.parser.parseScenario(text_str);
  ...
  getMessageInnerLayer: function () {
    ...
    if (this.stat.vchat.is_active) {
      const j_msg_inner = $("#vchat_base").find(".current_vchat"); //.find(".vchat-text-inner").html(current_str);
      //j_msg_inner.show();
      return j_msg_inner;
    } else {
  ...
  getMessageCurrentSpan: function () {
    //ここでも、
    const j_obj = this.layer.getLayer(this.stat.current_layer, this.stat.current_page).find(".message_inner").find("p").find(".current_span");
    return j_obj;
  },

  //即座に新しい領域を確保
  setMessageCurrentSpan: function () {
    const jtext = this.getMessageInnerLayer();
    const jtext_p = jtext.find("p");
    ...
    const current_span = jtext_p.find(".current_span");
    if (current_span.length > 0) {
      current_span.removeClass("current_span");
      this.stat.set_text_span = false;
    }

    const j_span = $("<span class='current_span'></span>");

    jtext_p.append(j_span); //縦書きの場合、ここに追加されてないかも
    ...
    if (ext == "mp3" || ext == "ogg" || ext == "m4a") {
      ...
      const howl_opt = {
        ...
        onloaderror: () => {
          ...
          // that.kag.error("Audio file 「" + src + "」 not found. Is the location specified with a full path? (Example: data/bgm/music.ogg)");
          if (callbk) callbk(obj);
        },
      };
      const obj = new Howl(howl_opt);
      ...
      $("<video />").attr("src", src).on("loadeddata", (function (e) {
        ...
        that.kag.error("Video file 「" + src + "」not found. Is the location specified with a full path? (Example: data/video/file.mp4)");
        callbk && callbk();
      }));
      ...
      $('<img />').attr('src', src).on("load", function (e) {
        ...
        that.kag.error("Image file 「" + src + "」 not found. Is the location specified with the full path? (Example: data/fgimage/file.png)");
  ...
  preloadAll: function (storage, callbk) {
    ...
    if (typeof storage == "object" && storage.length >= 0) {
      ...
      this.kag.preload(storage, function () {
        callbk();
      });
    }
  ...
  html: function (html_file_name, data, callback) {
    ...
    if (this.cache_html[html_file_name]) {
      if (callback) {
        const tmpl = $.templates(this.cache_html[html_file_name]);
      ...
      const path_html = this.kag.stat.sysview[html_file_name];

      $.loadText(path_html, function (text_str) {

        const tmpl = $.templates(text_str);
        const html = tmpl.render(data);
  ...
  error: function (error_message) {
    ...
    const err = `Error: ${current_storage}, line: ${line}\n${error_message}`;
  ...
  log: function (obj) {
    if (this.kag.config["debugMenu.visible"] == "true") {
      // console.log(obj); // DISABLED CONSOLE SPAMMING
    }
  },
