...
tyrano.plugin.kag.studio = {
  ...
  init: function () {
    ...
    if (window.navigator.userAgent.indexOf("TyranoStudio") != -1) {
      ...
      this.ipc.ipcRenderer.on("variable-add-all", (event, arg) => {
        ...
        const map_variable = TYRANO.kag.variable;

        const f = TYRANO.kag.stat.f;
        const mp = TYRANO.kag.stat.mp;
      ...
      //リロードボタンの配置
      const j_reload_button = $("<div style='position:absolute;z-index:999999999;padding:10px;opacity:0.8;background-color:white;left:0px;top:0px'><button style='cursor:pointer'><span style=''>" + $.lang("reload") + "</span></button></div>");
  ...
  insertElement: function (category, file) {
    ...
    if (category == "fgimage" || category == "image") {
      ...
      (() => {
        ...
        j_img.resizable({
          ...
          resize: (e, ui) => {
            ...
            const width = parseInt(target.css("width"));
            const height = parseInt(target.css("height"));
        ...
        _j_img.find(".button_delete").on("click", function () {
          _j_img.remove();
        });
      ...
      //背景画像変更
      const j_new_bg = TYRANO.kag.layer.getLayer("base", "fore");
  ...
  cutTyranoScript: function (str) {
    const result = TYRANO.kag.parser.parseScenario(str);

    const array_s = result.array_s;
