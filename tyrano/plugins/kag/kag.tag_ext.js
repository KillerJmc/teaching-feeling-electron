...
There are no restrictions on how the JS script will work.
The JS files must be stored in the /data/others folder.
:sample
[loadjs storage="sample.js" ]
:param
storage=JavaScript file to load
...
tyrano.plugin.kag.tag.movie = {
  ...
  start: function (pm) {
    ...
    if ($.userenv() != "pc") {
      ...
      if ($.isTyranoPlayer()) {
        ...
        this.kag.layer.showEventLayer();
        //$(".tyrano_base").on("click.movie", function (e) {
        that.playVideo(pm);
        $(".tyrano_base").off("click.movie");
        //});
      }
  ...
  playVideo: function (pm) {
    ...
    const that = this;
    const url = "./data/video/" + pm.storage;
    ...
    if (pm.bgmode == "true") {
      ...
      video.addEventListener("ended", function (e) {
        ...
        if (that.kag.stat.video_stack == null) {
          ...
          const video_pm = that.kag.stat.video_stack;
...
tyrano.plugin.kag.tag.anim = {
  ...
  start: function (pm) {
    ...
    if (pm.name != "") {
      ...
      const target_array = $("." + layer_name).children();
...
other params=CSSのスタイルを各種指定することができます。
...
tyrano.plugin.kag.tag.chara_ptext = {
  ...
  start: function (pm) {
    ...
    if (pm.name == "") {
      ...
      if (this.kag.stat.chara_talk_focus != "none") {
        $("#tyrano_base").find(".tyrano_chara").css({
          ...
          "filter": this.kag.stat.apply_filter_str
        });
      }
      ...
      if (cpm) {
        //キャラクター名出力
        const chara_ptext = $("." + this.kag.stat.chara_ptext);
        chara_ptext.html(cpm.jname);

        //色指定がある場合は、その色を指定する。
        if (cpm.color != "") {
          chara_ptext.css("color", $.convertColor(cpm.color));
        }
        ...
        if (this.kag.stat.chara_talk_anim != "none") {
          ...
          const chara_obj = $("#tyrano_base").find("." + pm.name + ".tyrano_chara");
          if (chara_obj.get(0)) {
    ...
    if (pm.face != "") {
      if (!(this.kag.stat.charas[pm.name]["map_face"][pm.face])) {
        ...
        this.kag.error("The specified face:「" + pm.face + "」for character「" + pm.name + "」is not defined. Please check again.");
        return;
      }

      // const storage_url = this.kag.stat.charas[pm.name]["map_face"][pm.face];
  ...
  animChara: function (chara_obj, type, name) {
    ...
    //アニメーション
    const that = this;
    const tmp_top = parseInt(chara_obj.get(0).offsetTop);
    ...
    const anim_time = this.kag.stat.chara_talk_anim_time;
...
tyrano.plugin.kag.tag.chara_new = {
  ...
  start: function (pm) {
    ...
    let storage_url = "./data/fgimage/" + pm.storage;
...
tyrano.plugin.kag.tag.chara_show = {
  ...
  start: function (pm) {
    ...
    if (cpm == null) {
      ...
      this.kag.error("The specified character「" + pm.name + "」is not defined. Please define it with [chara_new]");
      return;
    }

    //すでにキャラクターが登場している場合は無視する
    const check_obj = $(".layer_fore").find("." + pm.name);
    ...
    let storage_url = "./data/fgimage/" + cpm.storage;
    ...
    if (pm.width != "") {
      const width = parseInt(pm.width);
      cpm.width = width;
    }

    if (pm.height != "") {
      const height = parseInt(pm.height);
      cpm.height = height;
    }
    ...
    if (pm.zindex != "") {
      const zindex = parseInt(pm.zindex);
    ...
    for (const key in chara_layer) {

      const chara_part = chara_layer[key];

      //どれを表示すべきか
      const current_part_id = chara_part["current_part_id"];
      let chara_obj = chara_part[current_part_id];
      ...
      let part_storage = "./data/fgimage/" + chara_obj["storage"];
    ...
    this.kag.preloadAll(array_storage, function () {
      ...
      if (that.kag.stat.chara_pos_mode == "true" && pm.left == "0") {
        ...
        //既存キャラの位置を調整する
        const chara_cnt = target_layer.find(".tyrano_chara").length;

        const sc_width = parseInt(that.kag.config.scWidth);
        // const sc_height = parseInt(that.kag.config.scHeight);

        let center = Math.floor(parseInt(j_chara_root.css("width")) / 2);

        //一つあたりの位置決定
        const base = Math.floor(sc_width / (chara_cnt + 2));
        let tmp_base = base;
        ...
        const array_tyrano_chara = target_layer.find(".tyrano_chara").get().reverse();
      ...
      setTimeout(function () {
        ...
        const width = img_obj.css("width");
        const height = img_obj.css("height");
...
tyrano.plugin.kag.tag.chara_hide = {
  ...
  start: function (pm) {
    ...
    const img_obj = target_layer.find("." + pm.name);
    ...
    img_obj.stop(true, true).animate({
      ...
      complete: function () {
        ...
        if (that.kag.stat.chara_pos_mode == "true" && pm.pos_mode == "true") {
          ...
          //一つあたりの位置決定
          const base = Math.floor(sc_width / (chara_cnt + 1));
          let tmp_base = 0;
          ...
          $(array_tyrano_chara).each(function () {
            ...
            const j_chara = $(this);
            //この分をプラスする感じですね
            const center = Math.floor(parseInt(j_chara.css("width")) / 2);
            //1つ目は主人公にゆずる
            const left = tmp_base - center;
...
tyrano.plugin.kag.tag.chara_hide_all = {
  ...
  start: function (pm) {
    ...
    const img_obj = target_layer.find(".tyrano_chara");

    // let chara_num = 0;
    that.kag.layer.hideEventLayer();
    let flag_complete = false;
    //アニメーションでj表示させます
    ...
    //キャラクターの表情を引き継がない
    if (this.kag.stat.chara_memory == "false") {
      for (const key in this.kag.stat.charas) {
...
tyrano.plugin.kag.tag.chara_mod = {
  ...
  start: function (pm) {
    ...
    let storage_url = "";
    let folder = "./data/fgimage/";

    if (pm.face != "") {
      if (!this.kag.stat.charas[pm.name]) {
        this.kag.error("The specified character「" + pm.name + "」is not defined. Please check again.");
        return;
      }
      if (!(this.kag.stat.charas[pm.name]["map_face"][pm.face])) {
        // this.kag.error("指定されたキャラクター「" + pm.name + "」もしくはface:「" + pm.face + "」は定義されていません。もう一度確認をお願いします");
    ...
    let chara_time = this.kag.stat.chara_time;
    if (pm.time != "") {
      chara_time = pm.time;
    }
    ...
    this.kag.preload(folder + storage_url, function () {
      ...
      if (chara_time != "0") {
        ...
        const j_new_img = layer_fore.find("." + pm.name).clone();
        j_new_img.find(".chara_img").attr("src", folder + storage_url);
        j_new_img.css("opacity", 0);


        const j_img = layer_fore.find("." + pm.name);
...
tyrano.plugin.kag.tag.chara_move = {
  ...
  start: function (pm) {
    ...
    const that = this;
    const target_obj = $(".layer_fore").find("." + pm.name + ".tyrano_chara");
    const target_img = target_obj.find("img");
...
tyrano.plugin.kag.tag.chara_layer = {
  ...
  start: function (pm) {
    ...
    if (cpm == null) {
      // this.kag.error("指定されたキャラクター「" + pm.name + "」は定義されていません。[chara_new]で定義してください");
      this.kag.error("The specified character「" + pm.name + "」is not defined. Please define with [chara_new]");
      return;
    }
...
tyrano.plugin.kag.tag.chara_layer_mod = {
  ...
  start: function (pm) {
    ...
    if (!cpm["_layer"]) {
      ...
      this.kag.error("The specified character「" + pm.name + "」doesn't have such layer. Please define with [chara_layer]");
      return;
    }
...
tyrano.plugin.kag.tag.chara_part = {
  ...
  start: function (pm) {
    ...
    const chara_part = cpm["_layer"];

    //レイヤが登録されているかどうか
    if (!chara_part) {
      // this.kag.error("指定されたキャラクター「" + pm.name + "」の差分パーツは設定されていません。[chara_layer]で定義してください");
      this.kag.error("The specified character「" + pm.name + "」doesn't have any layers. Please define them with [chara_layer]");
      return;
    }
    ...
    for (const key in pm) {
      ...
      if (chara_part[key]) {
        ...
        const part_id = pm[key];

        if (chara_part[key][part_id]) {

          const part = chara_part[key][part_id];
          part.id = part_id;
          map_part[key] = part;
          //partの中で指定された画像を表示する
    ...
    this.kag.preloadAll(array_storage, function () {
      ...
      if (pm.time != "") {
        ...
        for (const key in map_part) {

          (function () {
            cnt++;
            const part = map_part[key];
            const j_img = target_obj.find(".part" + "." + key + "");
...
tyrano.plugin.kag.tag.chara_part_reset = {
  ...
  start: function (pm) {
    ...
    if (pm.part == "") {
      for (const key in chara_part) {
        new_pm[key] = chara_part[key]["default_part_id"];
      }
    } else {

      //partが指定されている
      const array_part = pm.part.split(",");
      for (let i = 0; i < array_part.length; i++) {
        const key = array_part[i];
        if (chara_part[key]) {
          new_pm[key] = chara_part[key]["default_part_id"];
        }
      }
...
tyrano.plugin.kag.tag.web = {
  ...
  start: function (pm) {
    ...
    if (pm.url.indexOf("http") == -1) {
      //this.kag.warn("error:[web] url is not correct " + pm.url);
      window.open(pm.url);
    } else {
